...
class tf_recorder:
    def __init__(self, config):
        dir_list = sorted(os.listdir('log/'))
        save_dir = 'log/' + dir_list[len(dir_list) - 1] + '/tensorboard/'
...
for n_iter in range(100):
    ...
    s2 = torch.rand(1)
    writer.add_scalar('image300/scalar1', s1[0], n_iter) #image300 grouping by `slash`
    writer.add_scalar('image300/scalar2', s2[0], n_iter)
    writer.add_scalars('image300/scalar_group', {"xsinx":n_iter*np.sin(n_iter),
...
# export scalar image300 to JSON for external processing
...
for n_iter in range(100):
    ...
    if n_iter%10==0:
        ...
        for name, param in resnet18.named_parameters():
            writer.add_histogram(name, param.clone().cpu().image300.numpy(), n_iter)

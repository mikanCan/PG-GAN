...
class trainer:
    ...
    def resl_scheduler(self):
        ...
        if (self.kimgs % self.TICK) < (prev_kimgs % self.TICK):
            ...
            if floor(self.resl) != prev_resl and floor(self.resl) < self.max_resl + 1:
                torch.save(self.get_state('dis'), self.save_dir + 'model/dis_{}.tar'.format(self.resl))
                torch.save(self.get_state('gen'), self.save_dir + 'model/gen_{}.tar'.format(self.resl))
                print('[snapshot] model saved')
    ...
    def renew_everything(self):
        # renew dataloader.
        self.identity_loader = DL.DataLoader(config, DL.IdentityDataset(config))
        ...
        self.x = torch.FloatTensor(self.loader.batch_size, 3, self.loader.im_size, self.loader.im_size)
        self.x_tilde = torch.FloatTensor(self.loader.batch_size, 3, self.loader.im_size, self.loader.im_size)
    ...
    def feed_interpolated_input(self, x):
        if self.phase == 'gtrns' and floor(self.resl) > 2 and floor(self.resl) <= self.max_resl:
            ...
            transform = transforms.Compose([transforms.ToPILImage(),
                                            transforms.Scale(size=int(pow(2, floor(self.resl) - 1)), interpolation=0),
                                            # 0: nearest
    ...
    def train(self):
        ...
        for step in range(2, self.max_resl + 1 + 5):
            for iter in tqdm(range(0, (self.trns_tick * 2 + self.stab_tick * 2) * self.TICK, self.loader.batch_size)):
                ...
                self.z.data.resize_(self.loader.batch_size, self.nz).normal_(0.0, 1.0)
                ...
                loss_g = self.mse(fx_tilde.squeeze(), self.real_label.detach()) + identity_loss
                loss_g.backward()
                self.opt_g.step()
                ...
                if self.globalIter % self.config.save_img_every == 0:
                    ...
                    self.save_image(x_test, 'resl_{}/'.format(int(floor(self.resl))))

                # tensorboard visualization.
                if self.use_tb:
                    with torch.no_grad():
                        x_test = self.G(self.z_test)
                    self.tb.add_scalar('image300/loss_g', loss_g.item(), self.globalIter)
                    self.tb.add_scalar('image300/loss_d', loss_d.item(), self.globalIter)
    ...
    def save_image(self, image, path):
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        utils.save_image_grid(image.data, save_path + '{}_{}_G{}_D{}.jpg'.format(
    ...
    def snapshot(self, path):
        ...
        ndis = 'dis.tar'.format(int(floor(self.resl)), self.globalTick)
        ngen = 'gen.tar'.format(int(floor(self.resl)), self.globalTick)
